...
from otherlayers import MaxLogLikePerSample, LowerBoundClipper
from hawkesprocess import HawkesProcessIntensity
import exogenousintensityfamily
import endogenousimpactfamily
import decaykernelfamily
...
class MixHawkesProcessModel(object):
    ...
    def __init__(self, num_type, num_cluster, num_sequence, mu_dict, alpha_dict, kernel_dict, activation, use_cuda):
        ...
        for m in range(self.num_cluster):
            ...
            exogenousIntensity = getattr(exogenousintensityfamily, mu_dict[k]['model_name'])
            k = min([m, len(alpha_dict) - 1])
            endogenousImpacts = getattr(endogenousimpactfamily, alpha_dict[k]['model_name'])
            k = min([m, len(kernel_dict) - 1])
            decayKernel = getattr(decaykernelfamily, kernel_dict[k]['model_name'])
